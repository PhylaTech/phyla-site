...
**IaC tool, when we need one:** **OpenTofu** (the open-source Terraform fork — avoids HashiCorp's
BSL licensing lock-in) with a small per-provider module; or **Pulumi** if we'd rather write infra
in TypeScript/Python. Adopt it alongside the *first real backend* (the Topicpile jobs, a database,
queues), not before.
